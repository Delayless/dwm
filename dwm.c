...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i){
            ...
            drw_rect(drw, x, bh - boxs, w, boxs, 1, 0);
		}
		x += w;
	}
...
hidewin(Client *c) {
	...
	focus(c->snext); /* if had a bug, changed c->snext to NULL. */
	arrange(c->mon);
}
...
runAutostart(void) {
	...
	system(STR(PROJECT_PATH)"/scripts/autostart_blocking.sh");
	system(STR(PROJECT_PATH)"/scripts/autostart.sh &");
}
